  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isScrolled ? 'glass-nav py-3 shadow-soft' : 'bg-background/95 backdrop-blur-md py-4 border-b border-surface-default'
      }`}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between">
          ...
          <Link href="/" className="flex items-center gap-3 group">
            <div className="w-11 h-11 sm:w-12 sm:h-12 rounded-full bg-primary/10 flex items-center justify-center text-primary group-hover:bg-primary group-hover:text-white transition-all duration-300 shadow-sm">
            ...
            <div>
              ...
              <span className="font-body text-[11px] sm:text-xs tracking-widest uppercase text-outline font-semibold mt-0.5 block">
                Nursery & Garden Care
              </span>
            </div>
          </Link>
            ...
            {navLinks.map((link) => {
              ...
              if (link.isMega) {
                return (
                  ...
                  >
                    <Link
                      href={link.href}
                      className={`relative inline-flex items-center gap-1.5 font-body text-sm font-medium transition-colors ${
                        isActive ? 'text-primary font-bold' : 'text-slate-700 hover:text-primary'
                      }`}
                    >
                      <span>{link.name}</span>
                      <ChevronDown className="w-4 h-4 text-slate-500 group-hover:text-primary transition-transform group-hover:rotate-180" />
                    ...
                    <AnimatePresence>
                      {productsMegaOpen && (
                        <motion.div
                          ...
                          className="absolute top-full -left-48 w-[820px] mt-1 bg-white rounded-card shadow-soft-lg border border-surface-default p-6 z-50 grid grid-cols-3 gap-6"
                        >
                          {CATEGORY_STRUCTURE.map((group) => (
                            <div key={group.groupSlug} className="space-y-3">
                              <div className="border-b border-surface-default pb-2">
                              ...
                              <div className="space-y-1">
                                {group.items.map((item) => (
                                  <Link
                                    ...
                                    className="flex items-start gap-2.5 p-2 rounded-lg hover:bg-surface-low text-slate-700 hover:text-primary transition-colors group/item"
                                  >
              ...
              return (
                <Link
                  key={link.name}
                  href={link.href}
                  className={`relative font-body text-sm font-medium py-2 transition-colors ${
                    isActive ? 'text-primary font-bold' : 'text-slate-700 hover:text-primary'
                  }`}
                >
                  <span>{link.name}</span>
          ...
          <div className="hidden sm:flex items-center gap-3">
            ...
            >
              <Button size="sm" variant="primary" icon={<Phone className="w-4 h-4" />}>
                WhatsApp Enquiry
              </Button>
            </a>
          </div>
          ...
          <div className="flex lg:hidden items-center gap-1.5">
            ...
            <button
              ...
              className="p-2 text-primary hover:bg-surface-low rounded-md transition-colors"
            >
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            ...
            className="lg:hidden bg-white border-b border-surface-default shadow-soft max-h-[85vh] overflow-y-auto"
          >
            <div className="px-4 py-6 space-y-3">
              <Link
                href="/"
                onClick={() => setMobileMenuOpen(false)}
                className="block font-body text-base font-semibold text-slate-800 hover:text-primary py-2 border-b border-surface-low"
              >
                Home
              </Link>

              {/* Mobile Products Accordion */}
              <div className="border-b border-surface-low py-2">
                <button
              ...
              <div className="pt-4 flex flex-col gap-3">
                <a
                  ...
                  className="w-full"
                >
                  <Button className="w-full" variant="primary" icon={<Phone className="w-4 h-4" />}>
